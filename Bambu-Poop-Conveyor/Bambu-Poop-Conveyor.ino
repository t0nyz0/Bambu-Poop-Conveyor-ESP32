...
int motorWaitTime = 10000; // The time to wait to run the motor. Once we see the printer is in a state that requires poop conveying, lets wait x amount of time to allow it to finish.
int delayAfterRun = 120000;
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("print") && doc["print"].containsKey("stg_cur")) {
    ...
    // Run on 4 (Change filament) or 1 (auto-bed level) - I've tried a few different statuses, attempting "14" with this build because its always run on Bambu instad of status 8 "filament calibration" which users can optionally turn off.
    if (printer_stage == 4 || printer_stage == 14 || printer_sub_stage == 4 && printer_stage != -1) {
      // About to poop
      if (debug) Serial.println("Time to run conveyor");
